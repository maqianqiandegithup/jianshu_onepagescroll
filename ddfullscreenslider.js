...
var ddfullscreenslider = (function($){
	...
	function supportstranslate3d(){ // based on http://stackoverflow.com/a/12621264/4360074
	  if (!window.getComputedStyle) {//这里是什么用法
	      return false;
	  }
	  ...
	  var has3d
		...
		transformprop = (findtransformprop)? findtransformprop[0] : 'transform'//这个不太懂是什么
	  $el.remove()
	...
	function ddfullscreenslider(setting){
		var translatesupport = supportstranslate3d()//判断这个是不是支持transform
		var s = $.extend({}, defaults, setting)//setting也是一个字典。
